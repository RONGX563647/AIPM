...
1. **表结构设计规范**（非SQL语句，统一范式）：
   ...
   - 关联字段：格式为「关联表名\_id」（如`project_id`关联`project`表），类型`bigint(20)`，加普通索引；
...
| 1.3        | Swagger接口验证                    | 测试登录接口入参/出参               | 输入admin/123456返回有效Token ✅（已验证，返回 JWT token） |
...
1. **表设计核心**：所有表以`id`为自增主键，关联字段用「表名\_id」，时间字段统一`datetime`，状态字段用明确枚举；
